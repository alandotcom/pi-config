 ...
 *            new Herdr tab → tmux split → new Ghostty window (macOS)
 *            → clipboard + printed command
 *
...
import { loadBtwConfig, parseModelSpec } from "./config.ts";
import { openFork } from "./launcher.ts";
import { firstLine, forkSessionWithThread, type Turn } from "./thread-fork.ts";
import {
  Box,
  type Component,
  Editor,
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand("btw", {
    ...
    handler: async (args, ctx) => {
      ...
      const configuredModel = modelSpec
        ...
        try {
          ...
          ctx.ui.notify(
            await openFork(
              ...
              `btw: ${firstLine(result.turns[0]!.question, 40)}`,
            ),
            "info",
          );
        } catch (err) {
